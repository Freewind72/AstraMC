...
try {
    ...
    // 检查是否有默认服务器地址
    $result = $db->query("SELECT server_address, server_name FROM server_settings_secondary ORDER BY id DESC LIMIT 1");
    if (!($row = $result->fetchArray(SQLITE3_ASSOC))) {
        // 插入默认服务器地址
        $defaultServerAddress = "mymcc.xin";
        $defaultServerName = "备用服务器";
