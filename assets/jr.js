function createDengContainer() {
    // 检查是否在首页，如果是则调整灯笼容器的z-index以避免遮挡导航栏
    ...
    // 如果在首页，调整灯笼容器的样式以避免遮挡重要元素
    if (isHomePage) {
        container.style.zIndex = '999';
        const boxes = container.querySelectorAll('.deng-box');
        boxes.forEach(box => {
...
function addStyles() {
    ...
    style.textContent = `
        ...
        @media (max-width: 768px) { 
            ...
            .deng-box { transform: scale(0.5); top: 100px; }  /* 移动端也调整位置 */
            .deng-box1 { left: -5%; }  
            .deng-box2 { left: 15%; }  
            .deng-box3 { right: 15%; }  
            .deng-box4 { right: -5%; }  
...
// 引入时调用
function init() {
    // 检查是否应该显示节日灯笼（可以根据日期或其他条件判断）
    // 这里可以添加一些条件判断，例如特定节日才显示
    addStyles();
    createDengContainer();
}
